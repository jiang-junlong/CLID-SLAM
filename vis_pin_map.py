...
def vis_pin_map(
    ...
    mesh_res_m: float = typer.Option(
        None, "--mesh_res_m", "-m", help="Resolution of the mesh in meters"
    ),
    cropped_ply_filename: str = typer.Option(
        "neural_points.ply",
        "--cropped_ply_filename",
        "-c",
        help="Path to the cropped point cloud file",
    ),
    output_mesh_filename: str = typer.Option(
        None, "--output_mesh_filename", "-o", help="Path to the output mesh file"
    ),
    mc_nn: int = typer.Option(
        9,
        "--mc_nn",
        "-n",
        help="Minimum number of neighbors for SDF querying for marching cubes",
    ),
    o3d_vis_on: bool = typer.Option(
        True, "--visualize_on", "-v", help="Turn on the visualizer"
    ),
):
    config = Config()
    ...
    sem_mlp = (
        Decoder(
            config,
            config.sem_mlp_hidden_dim,
            config.sem_mlp_level,
            config.sem_class_count + 1,
        )
    ...
    color_mlp = (
        Decoder(
            config,
            config.color_mlp_hidden_dim,
            config.color_mlp_level,
    ...
    mlp_dict = {"sdf": geo_mlp, "semantic": sem_mlp, "color": color_mlp}

    # initialize the neural point features
    neural_points: NeuralPoints = NeuralPoints(config)

    # Load the map
    loaded_model = torch.load(config.model_path, weights_only=False)
    ...
    neural_points.recreate_hash(
        neural_points.neural_points[0], torch.eye(3).cuda(), False, False
    )
    ...
    if mesh_on:
        ...
        if os.path.exists(cropped_ply_path):
            ...
            cropped_pc = neural_points.get_neural_points_o3d(
                query_global=True, random_down_ratio=23
            )
        ...
        while True:
            ...
            if not neural_points.is_empty():
                packet_to_vis.add_neural_points_data(
                    neural_points, only_local_map=False, pca_color_on=True
                )
