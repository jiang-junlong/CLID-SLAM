...
class Config:
    def __init__(self):
        ...
        self.use_kiss_dataloader: bool = (
            False  # use the dataloader providied by kiss-icp or not
        )
        ...
        self.tran_dtype = (
            torch.float64
        )  # dtype used for all the transformation and poses

        # dataset specific
        self.kitti_correction_on: bool = (
            False  # intrinsic vertical angle correction # issue 11
        )
        ...
        self.lidar_type_guess: str = "ouster128"
        ...
        self.rand_downsample: bool = (
            False  # apply random or voxel downsampling to input original point clcoud
        )
        ...
        self.color_channel: int = (
            0  # For RGB, channel=3, For Lidar with intensity, channel=1
        )
        ...
        self.idw_index: int = (
            2  # the index for IDW (inverse distance weighting), 2 means square inverse
        )
        self.buffer_size: int = int(
            5e7
        )  # buffer size for hashing, the smaller, the more likely to collision
        self.local_buffer_size: int = int(5e6)
        ...
        self.use_gaussian_pe: bool = (
            False  # use Gaussian Fourier or original log positional encoding
        )
        ...
        self.init_iter_ratio: int = (
            40  # train init_iter_ratio x iters for the first frame to kick the SLAM off
        )
        self.opt_adam: bool = (
            True  # use adam (default) or sgd as the gradient descent optimizer
        )
        ...
        self.photometric_loss_weight: float = (
            0.01  # weight for the photometric loss in tracking
        )
        self.consist_wieght_on: bool = True  # weight for color (intensity) consistency for the measured and queried value
        self.source_vox_down_m: float = (
            0.8  # downsample voxel resolution for source point cloud
        )

        self.reg_min_grad_norm: float = (
            0.5  # min norm of SDF gradient for valid source point
        )
        self.reg_max_grad_norm: float = (
            1.5  # max norm of SDF gradient for valid source point
        )
        ...
        self.reg_term_thre_deg: float = (
            0.01  # iteration termination criteria for rotation
        )
        self.reg_term_thre_m: float = (
            0.001  # iteration termination criteria for translation
        )
        ...
        self.global_loop_on: bool = (
            False  # global loop detection using context descriptor
        )
        self.local_map_context: bool = (
            False  # use local map context or scan context for loop closure description
        )
        self.loop_with_feature: bool = (
            False  # encode neural point feature in the context
        )
        ...
        self.local_loop_dist_thre: float = (
            2.0  # unit: m, find local loop within this distance
        )
        self.context_shape = [20, 60]  # ring, sector count for the descriptor
        self.npmc_max_dist: float = (
            60.0  # max distance for the neural point map context descriptor
        )
        self.context_num_candidates: int = 1  # select the best K candidates after comparing the ring key for further checking
        self.context_cosdist_threshold: float = (
            0.2  # cosine distance threshold for a candidate loop
        )
        ...
        self.pgo_with_pose_prior: bool = (
            False  # use the pose prior or not during the pgo
        )
        ...
        self.pgo_error_thre_frame: float = (
            500.0  # the maximum error for rejecting a wrong edge (per frame)
        )
        ...
        # eval
        self.wandb_vis_on: bool = (
            False  # monitor the training on weight and bias or not
        )
        ...
        self.o3d_vis_raw: bool = (
            False  # visualize the raw point cloud or the weight source point cloud
        )
        ...
        self.sdf_slice_height: float = (
            -1.0
        )  # initial height of the horizontal SDF slice (m) in sensor frame
        self.vis_sdf_res_m: float = (
            0.2  # resolution for the SDF slice for visualization (m)
        )
        ...
        self.vis_frame_axis_len: float = (
            0.8  # sensor frame axis length, for visualization, unit: m
        )
        ...
        # gui
        self.visualizer_split_width_ratio: float = (
            0.7  # the width ratio of the visualizer split window
        )
        ...
        self.save_map: bool = (
            False  # save the neural point map model and decoders or not
        )
        ...
        # ROS related
        self.publish_np_map: bool = (
            True  # only for Rviz visualization, publish neural point map
        )
        self.publish_np_map_down_rate_list = [
            11,
            23,
            37,
            53,
            ...
            131,
            151,
        ]  # prime number list, downsampling for boosting neural point map pubishing speed
        self.republish_raw_input: bool = (
            False  # publish the raw input point cloud or not
        )
        self.timeout_duration_s: int = (
            30  # in seconds, exit after receiving no topic for x seconds
        )
    ...
    def load(self, config_file):
        ...
        if "setting" in config_args:
            self.use_pin_mapper = config_args["setting"].get("use_pin_mapper", False)
            self.name = config_args["setting"].get("name", "clid_slam")
            self.use_kiss_dataloader = config_args["setting"].get(
                "use_kiss_icp_dataloader", False
            )

            self.output_root = config_args["setting"].get(
            ...
            self.semantic_on = config_args["setting"].get(
                "semantic_on", self.semantic_on
            )
            if self.semantic_on:
                self.label_path = config_args["setting"].get(
                    "label_path", "./demo_data/labels"
                )

            self.color_map_on = config_args["setting"].get(
                "color_map_on", self.color_map_on
            )
            self.color_channel = config_args["setting"].get("color_channel", 0)
            if (
                self.color_channel == 1 or self.color_channel == 3
            ) and self.color_map_on:
                self.color_on = True
            else:
                self.color_on = False
            ...
            self.first_frame_ref = config_args["setting"].get(
            ...
            self.device = config_args["setting"].get(
                "device", "cuda"
            )  # or cpu, on cpu it's about 5 times slower
            self.gpu_id = config_args["setting"].get("gpu_id", "0")

            self.kitti_correction_on = config_args["setting"].get(
                "kitti_correct", self.kitti_correction_on
            )
            if self.kitti_correction_on:
                self.correction_deg = config_args["setting"].get(
                    "correct_deg", self.correction_deg
                )
            self.stop_frame_thre = config_args["setting"].get(
                "stop_frame_thre", self.stop_frame_thre
            )

            self.deskew = config_args["setting"].get(
                "deskew", self.deskew
            )  # apply motion undistortion or not
            self.valid_ts_in_points = config_args["setting"].get(
                "valid_ts", self.valid_ts_in_points
            )
        ...
        if "process" in config_args:
            ...
            self.rand_downsample = config_args["process"].get(
                "rand_downsample", self.rand_downsample
            )
            if self.rand_downsample:
                self.rand_down_r = config_args["process"].get(
                    "rand_down_r", self.rand_down_r
                )
            else:
                self.vox_down_m = config_args["process"].get(
                    "vox_down_m", self.max_range * 1e-3
                )
            self.dynamic_filter_on = config_args["process"].get(
                "dynamic_filter_on", self.dynamic_filter_on
            )
            self.dynamic_sdf_ratio_thre = config_args["process"].get(
                "dynamic_sdf_ratio_thre", self.dynamic_sdf_ratio_thre
            )
            self.dynamic_certainty_thre = config_args["process"].get(
                "dynamic_certainty_thre", self.dynamic_certainty_thre
            )
            self.adaptive_range_on = config_args["process"].get(
                "adaptive_range_on", self.adaptive_range_on
            )

        # sampler
        if "sampler" in config_args:
            self.local_voxel_size_m = config_args["sampler"].get(
                "local_voxel_size_m", self.vox_down_m
            )
        ...
        if "neuralpoints" in config_args:
            self.voxel_size_m = config_args["neuralpoints"].get(
                "voxel_size_m", self.vox_down_m * 5.0
            )
            self.query_nn_k = config_args["neuralpoints"].get(
                "query_nn_k", self.query_nn_k
            )
            self.buffer_size = int(
                float(
                    config_args.get("neuralpoints", {}).get(
                        "buffer_size", self.buffer_size
                    )
                )
            )
            self.num_nei_cells = config_args["neuralpoints"].get(
                "num_nei_cells", self.num_nei_cells
            )
            self.layer_norm_on = config_args["neuralpoints"].get(
                "layer_norm_on", self.layer_norm_on
            )
            self.search_alpha = config_args["neuralpoints"].get(
                "search_alpha", self.search_alpha
            )
            self.feature_dim = config_args["neuralpoints"].get(
            ...
            self.weighted_first = config_args["neuralpoints"].get(
                "weighted_first", self.weighted_first
            )
            # build the neural point map from the surface samples or only the measurement points
            self.from_sample_points = config_args["neuralpoints"].get(
                "from_sample_points", self.from_sample_points
            )
            if self.from_sample_points:
                self.map_surface_ratio = config_args["neuralpoints"].get(
                    "map_surface_ratio", self.map_surface_ratio
                )
            self.prune_map_on = config_args["neuralpoints"].get(
                "prune_map_on", self.prune_map_on
            )
            self.max_prune_certainty = config_args["neuralpoints"].get(
                "max_prune_certainty", self.max_prune_certainty
            )
            self.use_mid_ts = config_args["neuralpoints"].get(
                "use_mid_ts", self.use_mid_ts
            )
            self.local_map_travel_dist_ratio = config_args["neuralpoints"].get(
                "local_map_travel_dist_ratio", self.local_map_travel_dist_ratio
            )
            self.window_radius = config_args["neuralpoints"].get(
                "window_radius", self.window_radius
            )
            self.local_map_radius = config_args["neuralpoints"].get(
                "local_map_radius", self.local_map_radius
            )

        # decoder
        if "decoder" in config_args:  # only on if indicated
            # number of the level of the mlp decoder
            self.geo_mlp_level = config_args["decoder"].get(
                "mlp_level", self.geo_mlp_level
            )
            ...
            self.freeze_after_frame = config_args["decoder"].get(
                "freeze_after_frame", self.freeze_after_frame
            )
        ...
        if "loss" in config_args:
            ...
            self.sigma_sigmoid_m = config_args["loss"].get(
                "sigma_sigmoid_m", self.vox_down_m
            )
        ...
        if "continual" in config_args:
            self.pool_capacity = int(
                float(config_args["continual"].get("pool_capacity", self.pool_capacity))
            )
            self.bs_new_sample = int(
                config_args["continual"].get(
                    "batch_size_new_sample", self.bs_new_sample
                )
            )
            self.new_certainty_thre = float(
                config_args["continual"].get(
                    "new_certainty_thre", self.new_certainty_thre
                )
            )
        ...
        if self.track_on:
            if self.color_on:
                self.photometric_loss_on = config_args["tracker"].get(
                    "photo_loss", self.photometric_loss_on
                )
                if self.photometric_loss_on:
                    self.photometric_loss_weight = float(
                        config_args["tracker"].get(
                            "photo_weight", self.photometric_loss_weight
                        )
                    )
                self.consist_wieght_on = config_args["tracker"].get(
                    "consist_wieght", self.consist_wieght_on
                )
            self.T_imu_lidar = torch.tensor(
                config_args["tracker"].get("T_imu_lidar", self.T_imu_lidar)
            )
            ba_value = config_args.get("tracker", {}).get("ba")
            if ba_value is not None:
                self.bg = torch.tensor(
                    config_args["tracker"].get("ba", self.ba),
                    device=self.device,
                    dtype=self.tran_dtype,
                )
            bg_value = config_args.get("tracker", {}).get("bg")
            if bg_value is not None:
                self.bg = torch.tensor(
                    config_args["tracker"].get("bg", self.bg),
                    device=self.device,
                    dtype=self.tran_dtype,
                )
            grav_value = config_args.get("tracker", {}).get("gravity")
            if grav_value is not None:
                self.gravity = torch.tensor(
                    config_args["tracker"].get("gravity", self.gravity),
                    device=self.device,
                    dtype=self.tran_dtype,
                )
            self.source_vox_down_m = config_args["tracker"].get(
                "source_vox_down_m", self.vox_down_m * 10
            )
            self.reg_iter_n = config_args["tracker"].get("iter_n", self.reg_iter_n)
            self.track_mask_query_nn_k = config_args["tracker"].get(
                "valid_nn_k", self.track_mask_query_nn_k
            )
            self.reg_min_grad_norm = config_args["tracker"].get(
                "min_grad_norm", self.reg_min_grad_norm
            )
            self.reg_max_grad_norm = config_args["tracker"].get(
                "max_grad_norm", self.reg_max_grad_norm
            )
            self.reg_term_thre_deg = float(
                config_args["tracker"].get("term_deg", self.reg_term_thre_deg)
            )
            self.reg_term_thre_m = float(
                config_args["tracker"].get("term_m", self.reg_term_thre_m)
            )
            self.measurement_noise_covariance = config_args["tracker"].get(
        ...
        if self.pgo_on:
            self.local_map_context = config_args["pgo"].get(
                "map_context", self.local_map_context
            )
            self.loop_with_feature = config_args["pgo"].get(
                "loop_with_feature", self.loop_with_feature
            )
            self.local_map_context_latency = config_args["pgo"].get(
                "local_map_latency", self.local_map_context_latency
            )
            self.context_virtual_side_count = config_args["pgo"].get(
                "virtual_side_count", self.context_virtual_side_count
            )
            self.context_virtual_step_m = config_args["pgo"].get(
                "virtual_step_m", self.voxel_size_m * 4.0
            )
            self.npmc_max_dist = config_args["pgo"].get(
                "npmc_max_dist", self.max_range * 0.7
            )
            self.pgo_freq = config_args["pgo"].get("pgo_freq_frame", self.pgo_freq)
            self.pgo_with_pose_prior = config_args["pgo"].get(
                "with_pose_prior", self.pgo_with_pose_prior
            )
            # default cov (constant for all the edges)
            self.pgo_tran_std = float(
                config_args["pgo"].get("tran_std", self.pgo_tran_std)
            )
            self.pgo_rot_std = float(
                config_args["pgo"].get("rot_std", self.pgo_rot_std)
            )
            ...
            self.pgo_error_thre = float(
                config_args["pgo"].get(
                    "pgo_error_thre_frame", self.pgo_error_thre_frame
                )
            )
            self.pgo_max_iter = config_args["pgo"].get(
                "pgo_max_iter", self.pgo_max_iter
            )
            self.pgo_merge_map = config_args["pgo"].get("merge_map", False)
            self.context_cosdist_threshold = config_args["pgo"].get(
                "context_cosdist", self.context_cosdist_threshold
            )
            self.min_loop_travel_dist_ratio = config_args["pgo"].get(
                "min_loop_travel_ratio", self.min_loop_travel_dist_ratio
            )
            self.loop_dist_drift_ratio_thre = config_args["pgo"].get(
                "max_loop_dist_ratio", self.loop_dist_drift_ratio_thre
            )
            self.local_loop_dist_thre = config_args["pgo"].get(
                "local_loop_dist_thre", self.voxel_size_m * 5.0
            )

        # mapping optimizer
        if "optimizer" in config_args:
            self.mapping_freq_frame = config_args["optimizer"].get(
                "mapping_freq_frame", 1
            )
            self.adaptive_iters = config_args["optimizer"].get(
                "adaptive_iters", self.adaptive_iters
            )
            self.iters = config_args["optimizer"].get(
                "iters", self.iters
            )  # mapping iters per frame
            self.init_iter_ratio = config_args[
                "optimizer"
            ].get(
                "init_iter_ratio", self.init_iter_ratio
            )  # iteration count ratio for the first frame (a kind of warm-up) #iter = init_iter_ratio*iter

            self.new_sample_ratio_more = config_args["optimizer"].get(
                "new_sample_ratio_more", self.new_sample_ratio_more
            )
            ...
            # bundle adjustment
            self.ba_freq_frame = config_args["optimizer"].get(
                "ba_freq_frame", 0
            )  # default off
            self.ba_frame = config_args["optimizer"].get(
                "ba_local_frame", self.ba_frame
            )
            self.lr_pose = float(
                config_args["optimizer"].get("lr_pose_ba", self.lr_pose)
            )
            ...
            if (
                self.ba_freq_frame > 0
            ):  # dirty fix to resolve the conflict of ba with stop frame
                self.stop_frame_thre = self.end_frame

        # vis and eval
        if "eval" in config_args:
            # use weight and bias to monitor the experiment or not
            self.wandb_vis_on = config_args["eval"].get(
                "wandb_vis_on", self.wandb_vis_on
            )
            ...
            self.local_map_default_on = config_args["eval"].get(
                "local_map_default_on", self.local_map_default_on
            )

            # frequency for pose log (per x frame)
            self.log_freq_frame = config_args["eval"].get(
                "log_freq_frame", self.log_freq_frame
            )
            # frequency for mesh reconstruction (per x frame)
            self.mesh_freq_frame = config_args["eval"].get(
                "mesh_freq_frame", self.mesh_freq_frame
            )
            # keep the previous reconstructed mesh in the visualizer or not
            self.keep_local_mesh = config_args["eval"].get(
                "keep_local_mesh", self.keep_local_mesh
            )
            # frequency for sdf slice visualization (per x frame)
            self.sdf_default_on = config_args["eval"].get(
                "sdf_default_on", self.sdf_default_on
            )
            self.sdfslice_freq_frame = config_args["eval"].get(
                "sdf_freq_frame", self.sdfslice_freq_frame
            )
            self.sdf_slice_height = config_args["eval"].get(
                "sdf_slice_height", self.sdf_slice_height
            )  # in sensor frame, unit: m

            # mesh related
            self.mesh_default_on = config_args["eval"].get(
                "mesh_default_on", self.mesh_default_on
            )
            self.mesh_min_nn = config_args["eval"].get("mesh_min_nn", self.mesh_min_nn)
            self.skip_top_voxel = config_args["eval"].get(
                "skip_top_voxel", self.skip_top_voxel
            )
            self.min_cluster_vertices = config_args["eval"].get(
                "min_cluster_vertices", self.min_cluster_vertices
            )
            self.mc_res_m = config_args["eval"].get(
                "mc_res_m", self.voxel_size_m
            )  # initial marching cubes grid sampling interval (unit: m)

            # save the map or not
            self.save_map = config_args["eval"].get("save_map", self.save_map)
            self.save_merged_pc = config_args["eval"].get(
                "save_merged_pc", self.save_merged_pc
            )
        ...
        self.window_radius = max(
            self.max_range, 6.0
        )  # for the sampling data poo, should not be too small
