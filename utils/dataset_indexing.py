...
def set_dataset_path(config: Config, dataset_name: str = "", seq: str = ""):
    ...
    if config.use_kiss_dataloader:
        ...
        if dataset_name == "kitti":
            ...
            config.pc_path = os.path.join(
                base_path, "sequences", seq, "velodyne"
            )  # input point cloud folder
            pose_file_name = seq + ".txt"
            config.pose_path = os.path.join(
                base_path, "poses", pose_file_name
            )  # input pose file
            config.calib_path = os.path.join(
                base_path, "sequences", seq, "calib.txt"
            )  # input calib file (to sensor frame)
            config.label_path = os.path.join(
                base_path, "sequences", seq, "labels"
            ...
            config.pc_path = os.path.join(
                base_path, seq, "Ouster"
            )  # input point cloud folder
            config.pose_path = os.path.join(
                base_path, seq, "poses.txt"
            )  # input pose file
        elif dataset_name == "kitti_carla":
            config.name = config.name + "_kitti_carla_" + seq
            base_path = config.pc_path.rsplit("/", 3)[0]
            config.pc_path = os.path.join(
                base_path, seq, "generated", "frames"
            )  # input point cloud folder
            config.pose_path = os.path.join(
                base_path, seq, "generated", "poses.txt"
            )  # input pose file
            config.calib_path = os.path.join(
                base_path, seq, "generated", "calib.txt"
            )  # input calib file (to sensor frame)
